...
INSERT INTO paciente_internacao (id, prontuario_id, data_internacao, data_alta, quarto_id, leito_id, motivo, profissional_responsavel_id, observacoes, hospital_unidade_id) VALUES (nextval('seq_internacao'), 1, now(), now() + interval '3 days', 1, 1, 'Observação', 1, 'Paciente em observação', 1);
...
INSERT INTO paciente_internacao (id, prontuario_id, data_internacao, data_alta, quarto_id, leito_id, motivo, profissional_responsavel_id, observacoes, hospital_unidade_id, data_cadastro, data_atualizacao)
SELECT nextval('seq_internacao'),
	   (SELECT id FROM paciente_prontuario ORDER BY random() LIMIT 1),
	   now() - ((floor(random()*1000))::int || ' days')::interval,
	   now() - ((floor(random()*900))::int || ' days')::interval,
	   (SELECT id FROM quarto ORDER BY random() LIMIT 1), (SELECT id FROM leito ORDER BY random() LIMIT 1),
	   'Motivo test' ,
